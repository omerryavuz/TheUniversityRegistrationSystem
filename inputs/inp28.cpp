...
int main(){
	try{
		...
		Offering* o6_1 = new  Offering(c6, 1, i2, 4, 2, 3) ;
		...
		int added9 = s1->addCourse(eee->getOffering(540,1)) ;

		cout << added1 << "\t" <<added4 << "\t"  << added6 << "\t" <<added7 <<"\t" <<added8<<"\t" <<added9<<endl ;

		ceng->addStudent(s1);
		ceng->removeStudent(s1->getID());
		cout << *s1<<endl;
